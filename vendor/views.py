...
def add_category(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            category.save()
            messages.success(request, 'Catégorie créé avec succès!')
...
def add_food(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            form.save()
            messages.success(request, 'Plat rajouté au Menu!')
    ...
    if request.user.is_authenticated:
        if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
            ...
            try:
                ...
                response = {'status': 'failed', 'message': from_hour+'-'+to_hour+' existe déjà pour ce jour!'}
                return JsonResponse(response)
        else:
